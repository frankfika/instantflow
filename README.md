...
瞬传是一款面向临时信息交换的网页工具。发送方选择文件或文字后生成 8 位一次性配对码，接收方在另一台设备上输入配对码即可建立连接。双方核对安全校验码后，内容才会在发送端加密、经服务端短暂中转，并在接收端本地解密。
...
- **安全校验码**：双方确认显示的校验码一致后才开始发送。
- **附加限制**：可要求相同网络出口，并可设置至少 8 位的接收口令。
- **用途隔离派生**：口令经随机盐和 PBKDF2-SHA-256（210,000 次）生成独立的验证与加密材料。
...
1. 发送设备选择文件或文字，按需设置有效期、网络限制和接收口令。
2. 生成一次性配对码，并在接收设备输入。
3. 两台设备核对相同的安全校验码。
4. 确认后完成加密传输；接收成功后临时数据自动销毁。
